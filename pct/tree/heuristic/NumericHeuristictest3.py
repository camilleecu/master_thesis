...
class NumericHeuristic3(Heuristic):
    def __init__(
        self,
        name,
        weights,
    ...
    def update_variance(self, i):
        ...
        if self.x.iloc[i] < self.split_thresholds[0]:  # left partition
            idx = 0
        elif self.x.iloc[i] > self.split_thresholds[1]:  # right partition
            idx = 2
    ...
    def variance_total(self):
        ...
        for i in range(3):
            variances[i] = self.variance(self.n_pos[i], self.k_pos[i], self.sv_pos[i], self.ss_pos[i])
