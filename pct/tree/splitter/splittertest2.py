...
from pct.tree.heuristic.NumericHeuristictest2 import NumericHeuristic2  # Updated import for NumericHeuristic2
...
class Splitter2:
    ...
    def numerical_split(self, x, y, instance_weights, attribute_name, criteria, return_index):
        """Finds the best split for the given numerical attribute using squared error."""
        ...
        # Initialize heuristic for NumericHeuristic2 (use squared error)
        splitting_value1 = 3  # Example threshold for LIKE / DISLIKE
        splitting_value2 = 0  # Example threshold for UNKNOWN

        # Pass splitting_value1 and splitting_value2 to NumericHeuristic2
        heuristic = NumericHeuristic2(self.criterion, self.target_weights, self.min_instances, self.ftest, 
                                      instance_weights, x_attribute, y, splitting_value1, splitting_value2)

        heuristic.update_missing()  # Update missing values
        highest_criterion = -np.inf  # Initialize highest criterion to an invalid value
        ...
        for i, v in enumerate(x_attribute):
            ...
            heuristic.update_squares_error(i)  # Update with squared error logic

            # Update the best criterion based on squared error (not variance)
            current_criterion = heuristic.squared_error_total()  # Use squared error instead of item error
            if current_criterion > highest_criterion:
                highest_criterion = current_criterion  # Update the highest criterion
    ...
    def categorical_split(self, x, y, instance_weights, attribute_name, criteria, return_index):
        ...
        # Construct the heuristic object for categorical attributes
        ...
        heuristic.squared_error_total()  # Use squared error instead of variance
