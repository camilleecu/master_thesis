...
class Tree:
    ...
    def build(self, x, y, instance_weights, parent_node, depth=0):
        ...
        lovers = [u for u in users_rated_item if dict(rU_subset[u]).get(best_item, 0) >= 81]
        haters = [u for u in users_rated_item if dict(rU_subset[u]).get(best_item, 0) <= 80]
    ...
    def predict_instance(self, instance, node, res: dict):
        ...
        rating = instance.get(item_id, 0)  # instance rating at current node
        
        if hasattr(node, 'ri'):
            for item, user_ratings in node.ri.items():
                if len(user_ratings) > 4:
                    res[item] = np.mean([rating for user, rating in user_ratings])
        if item_id is None or node.is_leaf:
            return res
            
        if pd.isna(rating) or rating == 0:
            self.predict_instance(instance, node.children[2], res)  # Unknowns
        elif rating >= 4:
            self.predict_instance(instance, node.children[0], res)  # Lovers
        else:
            self.predict_instance(instance, node.children[1], res)  # Haters
        return res
    ...
    @staticmethod
    def calculate_mse(predict: np.ndarray, target: pd.DataFrame):
        """计算预测结果与gt的mse,忽略target为0即没有gt的ui对"""
        nonzeroMask = target != 0
        target_nonzero = target.values[nonzeroMask.values]
        predict_nonzero = predict[nonzeroMask.values]
        mse = np.mean((target_nonzero - predict_nonzero) ** 2)
        return mse

    def calculate_rmse_df(y_pred: pd.DataFrame, y_test: pd.DataFrame) -> float:
        """
        计算 RMSE，只在 y_test 非零的位置上计算，并跳过 y_pred 为 NaN 的位置。

        参数:
            y_pred: 模型预测出的评分 DataFrame
            y_test: 测试集中真实评分的 DataFrame（其他为 0）

        返回:
            float: 预测评分与真实评分之间的 RMSE
        """
        # 找出 y_test 中非零评分位置的掩码
        test_mask = y_test != 0

        # 只保留这部分预测和真实值
        y_test_valid = y_test[test_mask]
        y_pred_valid = y_pred[test_mask]

        # 移除预测为 NaN 的位置（模型可能没覆盖）
        mask_not_nan = ~y_pred_valid.isna()
        y_test_final = y_test_valid[mask_not_nan]
        y_pred_final = y_pred_valid[mask_not_nan]

        # 拉平成一维数组
        y_true = y_test_final.values.flatten()
        y_pred_vals = y_pred_final.values.flatten()

        if len(y_true) == 0:
            print("⚠️ 没有有效的 ground truth 和预测对来计算 RMSE")
            return float("nan")
