...
from pct.tree.splitter.splittertest2 import Splitter2  # Updated import
...
class Tree2:
    ...
    def fit(self, x, y, target_weights=None):
        ...
        instance_weights = pd.DataFrame(np.full(x.shape[0], Tree2.INITIAL_WEIGHT), index=x.index)
        ...
        self.splitter2 = self.make_splitter()
    ...
    def build(self, x, y, instance_weights, parent_node): 
            ...
            attribute_name, criterion_value, attribute_value = self.splitter2.find_split(x, y, instance_weights)
            ...
            x_middle = x.loc[middle_subset.index]
            x_right = x.loc[right_subset.index]
            ...
            node.children = [
                    self.build(x_left, y_left, instance_weights_left, node),  # Left child
                    self.build(x_middle, y_middle, instance_weights_middle, node),  # Middle child
                    self.build(x_right, y_right, instance_weights_right, node)  # Right child
            ]
